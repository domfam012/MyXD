...
const Update = props => {
  ...
  const [link, setLink] = useState(data.movUrl);
  ...
  const handleSubmit = async e => {
    ...
    if (check) {
      ...
      // console.log(`${process.env.ASSET_PREFIX}/api/lecture/post/${pid}`);

      axios
        .patch(`${process.env.ASSET_PREFIX}/api/lecture/post/${pid}`, reqData, {
          headers: {
...
Update.getInitialProps = async ctx => {
  ...
  const res = await fetch(
    `${process.env.ASSET_PREFIX}/api/lecture/post/${pid}`
  );
