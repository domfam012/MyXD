    ...
    <Layout>
      <Head>
        ...
        <meta
          ...
          content={`Adobe XD 템플릿 - ${data.title}`}
        />
        ...
        <meta
          ...
          content={`${data.title}. Adobe XD 무료 템플릿을 받아보세요.`}
        />
      </Head>
...
Detail.getInitialProps = async ctx => {
  ...
  const asideRes = await fetch(
    `${process.env.ASSET_PREFIX}/api/board/interest`
  );
  const asideResult = await asideRes.json();

  const relatedRes = await fetch(
    `${process.env.ASSET_PREFIX}/api/board/related?pid=${pid}&category=${cat}`
  );
