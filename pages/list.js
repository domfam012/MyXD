    ...
    <Layout page={"list"} activeMenu={activeMenu}>
      <Head>
        ...
        <meta
          ...
          content={`어도비 XD - My Adobe XD ${category}`}
        />
        <meta
          name="description"
          content={`Adobe XD ${category} 템플릿 - MyXD`}
        />
        <meta
          name="keywords"
          content={`어도비 Adobe XD ${category} ${activeMenu}`}
        />
...
Index.getInitialProps = async function(ctx) {
  ...
  const res = await fetch(
    `${process.env.ASSET_PREFIX}/api/board/list/15?cat=${cat}&page=${page}`
  );
